...
# check rate of PTERM2 : there seems to be a probleme there. 

PTERM2 <- list ( N = D1*IOM$N, rate = fi, phase = IOM$delta)
PTERM3 <- list ( N = D2*IOM$N^2, rate = 2*fi, phase = 2*IOM$delta)
PTERM4 <- list ( N = D2ik*outer(IOM$N, IOM$N), 
                       rate = outer(fi, fi, '+'), 
                       phase = outer(IOM$delta, IOM$delta, '+'))

## ATTENTION !!! Coriger les phases: c'est pas IOM$delta mais deltaprime = delta + zeta. 
...
par(mfrow=c(1,1))
plot(PSI_ORDERED$rate, PSI_ORDERED$N, type='h')

require(palinsol)
data(BER90)

par(mfrow=c(1,2))
BRate <- BER90$Table5$Rate*(2*pi)/360/3600
BAmp <- BER90$Table5$Amp*(2*pi)/360/3600

plot(PSI_ORDERED$rate, PSI_ORDERED$N, type='h')
plot(BRate, BAmp, type='h')


# par(mfrow=c(1,2))
# 
# plot(PSI_ORDERED$rate, abs(PSI_ORDERED$N), type='n')
# lines( abs(PTERM1$rate), abs(PTERM1$N), type='h', col=1)
# lines(abs(PTERM2$rate), abs(PTERM2$N), type='h', col=2)
# lines(abs(PTERM3$rate), abs(PTERM3$N), type='h', col=3)
# lines(abs(PTERM4$rate), abs(PTERM4$N), type='h', col=4)
# lines(abs(PTERM5$rate), abs(PTERM5$N), type='h', col=5)
